...
def main():
    ...
    for k in range(no_run):
        ...
        if (magix_run):
            ...
            # t_pred, xrecon = magix_model.predict(tdata[irecon], tdata[:1], xinfer[:1,:])
            t_pred, xrecon = magix_model.predict(
            tp=tdata[irecon][1:],  # prediction times (excluding initial time)
            t0=tdata[irecon][:1],  # initial time
            x0=xinfer[:1,:],       # initial state
            random=False
            )
