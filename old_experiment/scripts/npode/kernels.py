...
from gpflow.utilities import positive     # <-- use this
# Or, if you prefer pure TFP:
...
class Kernel:
    def __init__(
        ...
        with tf.name_scope(name):
            # now using positive() instead of gpflow.transforms.Log1pe()
            sf_param = Param(
                sf0,
                transform=positive(),
                name="sf",
        ...
        # store the Param *objects* (so you can re-assign later)
        self._sf_param = sf_param
        self._ell_param = ell_param
        self.fix_sf = fix_sf
    ...
    def sf(self):
        ...
        return self._sf_param()

    @property
    def ell(self):
        return self._ell_param()
